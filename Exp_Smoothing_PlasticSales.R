...
plastics<-read.csv(file.choose()) # plasticSales.csv
View(plastics)

# Converting data into time series object
amts<-ts(plastics$Sales,frequency = 12,start=c(60))
View(amts)

# dividing entire data into training and testing data 
train<-amts[1:48] # 36 months
test<-amts[49:60] # 12 months
...
MA # MAPE of Training 4.06 & Test 21.02
plot(Forecasted)
...
hw_nabg # alpha=0.78,beta=0,gamma=0.81
...
##############################################################
# Based on the MAPE TABLE-> MAPE of 'hwnabg' is minimum (9.83). 
# Build new model
...
# Based on the above MAPE TABLE NEW-> MAPE of 'hwnabg_mape_new' is minimum (8.60).
#Build new model
