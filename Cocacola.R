...
cocacola$Q1<-as.factor(cocacola$Q1)
cocacola$Q2<-as.factor(cocacola$Q2)
cocacola$Q3<-as.factor(cocacola$Q3)
cocacola$Q4<-as.factor(cocacola$Q4)

str(cocacola)
attach(cocacola)

# Split the data into train & test dataset
train<-cocacola[1:38,] # 38 Qtrs
test<-cocacola[39:42,] # 4 Qtrs

########################### LINEAR MODEL #############################

linear_model<-lm(Sales~t,data=train)
summary(linear_model) # R^2: 0.811
...
summary(add_sea_model) #R^2:0.096
...
summary(multi_sea_model) # R^2:0.104
...
summary(multi_sea_linear_model) # R^2:0.92
...
# Multi seasonality with Linear has least RMSE value (225.52)
# Build new model on complete data (train+test)

new_model<-lm(log_Sales~t+Q1+Q2+Q3+Q4,data=cocacola )
exp(new_model$fitted.values)
summary(new_model) # R^2: 0.94

RMSE_new_model<-sqrt(mean((cocacola$Sales-exp(new_model$fitted.values))^2,na.rm = T))
RMSE_new_model # 186.07
...
test_data$Q2<-as.factor(test_data$Q1)
test_data$Q3<-as.factor(test_data$Q1)
test_data$Q4<-as.factor(test_data$Q1)
...
write.csv(test_data,file="Cocacola_Forecast.csv",row.names = F)
getwd()
